# Context-based Adaptive Range Compressor

Lossless compressor for grayscale astronomical images (8-bit and 16-bit RAW). It
combines an **adaptive range coder** with **configurable context modelling**,
where the neighbourhood used to predict each symbol is described by an external
configuration file.

This is the single-threaded range-coding variant of the mpaic family. See also:

- [mpaic-range-mt](../mpaic-range-mt/README.md) - multithreaded range variant
...
It is functionally equivalent to [mpaic-arith](../mpaic-arith/README.md); the
difference is the entropy backend: mpaic-range uses a **carryless range coder**
(Subbotin-style) instead of a bit-oriented arithmetic coder. This typically makes
encoding/decoding faster with a comparable compression ratio.
...
If no output file is given, one is derived automatically with the `.crx`
...
```bash
./encoder image.raw out.crx -r 1024 -c 1024 -b 16 -e le -f context_default.txt
```
...
OUTPUT_ROOT=/tmp/crcx-results ./test.sh path/to/dataset
```
...
| Payload | range-coded bytestream |
