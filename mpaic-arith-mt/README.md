# Multithreaded Context-based Adaptive Arithmetic Compressor
...
and adds **multithreaded band-parallel encoding**.

This is the multithreaded arithmetic-coding variant of the mpaic family. See also:

- [mpaic-arith](../mpaic-arith/README.md) - single-threaded arithmetic variant
...
context configuration are identical to [v4 CACX](../cacx-single-threaded/README.md).
...
single-threaded [v4 CACX](../cacx-single-threaded/README.md).
...
If no output file is given, one is derived automatically with the `.caxmt`
...
```bash
./encoder image.raw out.caxmt -r 1024 -c 1024 -b 16 -e le -t 8 -f context_default.txt
```
...
THREADS=8 OUTPUT_ROOT=/tmp/cacx-mt-results ./test.sh path/to/dataset
```
...
| Payload | concatenated arithmetic-coded band streams |
