...
context_line=$(
python3 - "$SCRIPT_DIR/src/encoder.cpp" <<'PY'
import sys, re
try:
    ...
    print("[HARDCODED] context=default")
PY
)
fi
...
for input_file in "${INPUT_FILES[@]}"; do
    ...
    base="${filename%.raw}"
    output_file="$OUTPUT_DIR/${base}.cax"
    decoded_file="$OUTPUT_DIR/${base}.decoded.raw"

    enc_err_file=$(mktemp)
    enc_stats=$("$ENCODER" "$input_file" "$output_file" "${CTX_ARGS[@]}" 2> "$enc_err_file")
    enc_status=$?
    ...
    dec_stats=$("$DECODER" "$output_file" "$decoded_file" -c)
    dec_status=$?
